...
import { AdminPayload, AdminRequest } from '../admin-request.interface';

/**
 * 管理员守卫：解析 Authorization: Bearer <admin accessToken>。
 * - 未携带 token → 20001 未登录
 * - token 无效 / 过期 / 类型非 admin-access → 40101 token 过期或无效
 * 校验通过后把 admin 载荷写入 req.admin。
 */
@Injectable()
